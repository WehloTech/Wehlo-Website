...
export default function AboutUs() {
    ...
    const { founders = [] } = usePage().props;

    const mainFounder = founders[0];
    const otherFounders = founders.slice(1); // ✅ ALL remaining founders
        ...
        >
            ...
            <div className="w-full space-y-3">
                <div className="w-full text-2xl font-bold border-b-2">
                    {highlightText(founder.name, searchQuery)}
                </div>

                <div className="text-blue-700">
                    {highlightText(founder.position || "Founder", searchQuery)}
                </div>
            </div>
        </div>
    ))}
